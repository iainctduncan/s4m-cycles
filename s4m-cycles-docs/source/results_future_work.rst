...
Overall, I felt the system and performance were successfull. 
It achieved goals I set out in various ways:

Saxophone As Input Source
^^^^^^^^^^^^^^^^^^^^^^^^^
The first artistic goals was to create pieces starting with no preprogrammed material, 
and in which all material came from the saxophone, and in which the performer
...
watches than I have previously been while manipulating dials and the like.

To me, this was a very satisfying way to create music and is a direction I will
certainly continue to explore in future works. I belive it was also a more
...
Additionally, the structure becomes very clearly strophic.
...
In face, in conversation with others after the performance, I heard that people could
not tell that looping was even going on.
...
The tiiming in sequenced lines is determined both my my timing as a player and by the
main clock tempo and loop point (4 bars at 70 bpm in this performance). 
In performance, the sequenced lines  have clear temporal connection to what I have played, but as 
my playing is not (after the beginning) synchronized with the main clock, there
is drift and rhythmic surprise. 
...
pushing me to play material different than I would in a solo performance.
This was also successful, with the transformations and temporal elasticity providing
an effect not disimilar to playing with other performers in a free improvisation context.
...
within a strophic form and then removing them similarly. 
...
I have numerous ideas and plans for improving system and process of creating music with it.

Tracking Improvements
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
The principal deficiency in the tracker is that it does note consider a change of pitch 
within a legato line to be a new note. Fixing this will require significant code
...
Dynamic Range
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
I am able to acheive dynamic variety through the volume of sound and the 
density of note probability, however I found that in performance I wanted to 
be able to bring the overall intensity down more quickly. 
...
their lifecycles. While the number of notes playing can be changed with
the foot controlller, they still go on for a long time.
...
breaking connection with the audience.
...
I believe a rich area of improvement will be mapping the physical oreintation
...
Where I to change the microphone to a clip-on mic, and implement some physical 
tracking, I could influence the algorithms and mix automation effectively
with multiple streams based on the position of the mouthpeice and bell
and their orientation to each other.

Audio Controls 
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Another area that I have not explored in this version is having the note material
capture by the system change algorithmic parameter or system behaviour based
on the notes themselves. For example, pitches within the last X pitches could be
used to determine key centers, or measurements of the density of captured
...
whether a click track that is inaudible to the audience or through a visual
...
pulse changes coming from onset detection and gestural controls.

Additionally, the sequencers all use the same clock at present. 
...
perhaps variously assigning people to programmatic alterations or improvistions.
I also think composite captures weaving together input from multiple sources
into single lines could be fruitful and provide surprise.
...
success, providing me with a future direction that is both artisitically exciting
and will be supported by novel work highlighting the advantages of working in Scheme for Max.

While far from perfect, the tracking layer and system performance was responsive enough
...
As an improvisor, I also felt engaged with and excited by the work much more than I have
for projects where I provide a composition but do not perform.
